...
| `get_form_answers` | `pivot` — opcionais: `edge_id`, `format` | As respostas de um formulário, dentro de `response` |
...
<Tab title="Respostas de um formulário">
  `get_form_answers` precisa do `pivot`, que vem de `project_dynamic_forms`. As duas consultas se encadeiam:
