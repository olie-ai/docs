...
<Accordion title="Fluxo de execução" icon="diagram-project">
  ...
  | Dados extras | O que a ação produziu — identificadores criados, valores calculados |
  | Condição | Nas condições, se o resultado foi verdadeiro ou falso |

  <Note>
  Na ação **Disparar webhook**, o que o endpoint devolve **não volta para a automação**: o corpo da resposta não vira variável e não entra em uma condição dos passos seguintes. Do retorno, o fluxo aproveita apenas se a chamada respondeu 200 — e é isso que aparece como sucesso ou erro do componente. Ver [Disparar webhook](/guides/automation/actions-integrations#disparar-webhook).
  </Note>
</Accordion>
...
<Step title="O componente rodou?" icon="list-check">
  Procure o componente no fluxo de execução. Se ele não aparece, o fluxo não chegou nele: uma condição anterior bloqueou o fluxo, a cadeia de automações atingiu o limite, ou ele está dentro de um ramo que não foi executado.
</Step>
...
<Accordion title="Comunicação" icon="paper-plane">
  ...
  | `channel_counterpart_not_found` | O destinatário ficou em branco e o projeto não tem conversa nessa integração | Preencha o destinatário |
  | `channel_counterpart_ambiguous` | O destinatário ficou em branco e o projeto conversa com mais de um contato nessa integração | Preencha o destinatário para escolher um deles |
  | `channel_counterpart_unresolved` | A expressão do destinatário não devolveu nada | Confira a variável usada; a ação não recorre à conversa do projeto |
</Accordion>
...
<Accordion title="Projetos alvo" icon="sitemap">
  | Código | Significado | O que fazer |
  | --- | --- | --- |
  | `project_scope_resolution_failed` | A lista de projetos alvo não pôde ser montada — a expressão de sintaxe avançada falhou ou o filtro ficou inválido | Revise a escolha em **Definir projetos alvo** ou **Definir em quais projetos** |
  | `scoped_action_iteration_failed` / `scoped_condition_iteration_failed` | O componente falhou em um dos projetos da lista; quando a falha tem um código próprio, é ele que aparece | Leia a mensagem daquele projeto; os demais continuam sendo processados |
  | `iteration_failed` | Marca a execução quando algum projeto da lista falhou ou quando a lista passou de 50 projetos e foi cortada | Procure no fluxo o componente com erro; veja [Em quais projetos cada componente age](/guides/automation/anatomy#em-quais-projetos-cada-componente-age) |
</Accordion>
...
<Accordion title="A automação parou no meio" icon="circle-stop">
  Duas causas possíveis, e o log distingue as duas:

  - Uma condição com **bloquear o fluxo** deu falso.
  - A cadeia de automações atingiu o limite de cinco níveis.

  Um erro em um componente não para a automação: ele fica registrado naquele componente, a execução é marcada com erro e os componentes seguintes continuam rodando.
</Accordion>
...
<Accordion title="Tudo deu sucesso mas nada mudou" icon="circle-check">
  ...
  Com [projetos alvo](/guides/automation/anatomy#em-quais-projetos-cada-componente-age) definidos, a mensagem `no_projects_in_scope` indica que a lista ficou vazia — os irmãos de um projeto sem pai, por exemplo — e a ação não tinha onde agir.
</Accordion>
