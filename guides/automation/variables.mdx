...
| `get_form_answers` | [`pivot`](/api-reference/general/form-answer-pivot) | As respostas de um formulário específico |
...
<Warning>
`project_medias` devolve o **conteúdo publicado no projeto** — a aba **Conteúdo**. Ela não alcança o **Caderno**, que é outro recurso. Nenhuma consulta lê páginas do Caderno; veja [Caderno em automações e integrações](/guides/notebook/automation-and-api).
</Warning>

<Card title="A função query em detalhe" icon="magnifying-glass" href="/guides/advanced/advanced-syntax#funções-de-consulta">
  Parâmetros opcionais, encadeamento de consultas, limites e o que fazer quando a consulta não devolve nada.
</Card>
