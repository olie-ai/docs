...
<Warning>
O projeto criado pode acionar outras automações — criado direto numa etapa, por exemplo, ele dispara as automações daquela etapa. A mesma automação não é acionada de novo enquanto ainda está rodando, mas duas automações diferentes podem formar um laço. O [limite de encadeamento](/guides/automation/anatomy#automações-que-acionam-automações) interrompe a cadeia, mas cada volta gasta execuções do plano. Use condições para garantir que o projeto criado não se encaixe no gatilho da outra automação.
</Warning>
