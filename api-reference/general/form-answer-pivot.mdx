...
<Note>
Esta página trata de **respostas**. Criar ou alterar a definição do formulário — os campos, os tipos, as condicionais — é outro endpoint: ver [Criar e editar formulários pela API](/guides/forms/forms-api).
</Note>
...
### Quais objetos aceitam formulário

Só **quatro** objetos aceitam formulário de objeto — aquele que é configurado uma vez, por conta, e passa a valer para todos os registros daquele tipo:

| Objeto | `class` em `set-model-forms` |
| --- | --- |
| **Projeto** | `App\Models\Project` |
| **Cliente** | `App\Models\Customer` |
| **Contato** | `App\Models\Contact` |
| **Vínculo** — o usuário dentro da conta | `App\Models\FrameRelationship` |

Cada um aceita **um** formulário. A configuração é feita em `POST /api/management/set-model-forms`, que exige a permissão `frame_settings.edit.model_forms`.

<Warning>
**Squad não aceita formulário.** O objeto aparece na tela **Formulário dos objetos**, mas não tem suporte por trás: enviar `App\Models\Squad` em `set-model-forms` devolve erro `class_unable_for_forms`.
</Warning>

<Info>
Os outros `pivot_class` da tabela acima — funil, etapa e formulário avulso — não são configuração de objeto. São **vínculos internos** que a plataforma cria sozinha quando o projeto entra no funil, quando chega à etapa, ou quando alguém anexa um avulso. Você não os cria pela API: apenas os endereça para ler e gravar respostas.

O mesmo vale para `App\Models\FormPublicationResponse`, que endereça a resposta de uma [publicação de formulário](/guides/forms/public-forms) e pode aparecer no campo `model` de uma leitura.
</Info>
...
<Frame caption="O menu do cartão de formulário, com a opção Copiar pivô como JSON">
  <img src="/images/api-reference/copiar-pivot-como-json.png" alt="Cartão do formulário Qualificação do lead, do funil BACKEND, com o menu aberto mostrando os grupos Formulário, Versões e Opções avançadas, este último com Copiar pivô como JSON e Gerar link de resposta" />
</Frame>
...
Cada item precisa de `id` — o identificador do campo, obtido na leitura — e `answer`. Campos não enviados permanecem como estavam. O formato de `answer` depende do tipo do campo. Ver [Formato da resposta de cada campo](/guides/forms/answers-format).
</ParamField>

<Note>
Campo do tipo **anexo** tem um passo a mais: o arquivo sobe antes, em outro endpoint, e o objeto devolvido é que vira o `answer`. Ver [Anexos em resposta de formulário](/guides/forms/attachments).
</Note>
